    ...
    <div class="bg-white shadow overflow-hidden">
      ...
      <div class="flex flex-col md:flex-row md:justify-between md:items-center px-6 py-4 gap-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-4 justify-center mx-auto">
          <div class="text-sm">
            Show
            <select v-model="perPage" @change="fetchLayanans" class="border border-gray-300 px-2 py-1 mx-1 text-sm focus:outline-none focus:ring-1 focus:ring-gray-300">
      ...
      <div class="p-5">
        <div class="overflow-x-auto">
          <table class="w-full text-xs text-left text-gray-800">
            ...
            <tbody>
              <tr v-for="(layanan, index) in layanans" :key="layanan.id_layanan" :class="[index % 2 === 0 ? 'bg-gray-50' : 'bg-white', 'hover:bg-gray-100']">
        ...
        <div class="bg-white w-full max-w-md rounded shadow-lg p-6">
          <div class="flex justify-between items-center mb-4 border-b pb-2">
            <h2 class="text-lg font-semibold">
              {{ modalMode === 'edit' ? 'Edit Layanan' : 'Tambah Layanan' }}
            </h2>
...
const columns = [
  { key: 'id_layanan', label: 'No' },
  { key: 'group_layanan', label: 'Group Layanan' },
...
const fetchLayanans = async (useGlobalLoader = true) => {
...
const submitForm = async () => {
  ...
  try {
    if (modalMode.value === 'edit') {
      await axiosInstance.put(`/layanans/${form.value.id_layanan}`, form.value);
    } else {
      await axiosInstance.post('/layanans', form.value);
    }
...
const deleteLayanan = async (id) => {
  ...
  globalLoading.value = true;
  try {
    await axiosInstance.delete(`/layanans/${id}`);
    await fetchLayanans();
  } catch (error) {
...
onMounted(fetchLayanans)
watch([page, perPage], fetchLayanans)
