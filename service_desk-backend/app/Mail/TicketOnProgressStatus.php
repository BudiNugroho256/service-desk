...
class TicketOnProgressStatus extends Mailable
{
    ...
    public function build()
    {
        ...
        $pic = $this->ticket->escalationTo?->nama_user ?? $this->ticket->pic?->nama_user ?? 'Belum Ada';
        $isEscalated = !empty($this->ticket->escalation_to);
        $priorityDays = $this->ticket->priority
            ? ($isEscalated ? $this->ticket->priority->sla_duration_escalation : $this->ticket->priority->sla_duration_normal)
            : 0;
        ...
        $body = "
            <h2>🚧 Tiket Sedang Dikerjakan - {$this->ticket->id_ticket_type}</h2>
            <p>Halo <strong>{$userName}</strong>,</p>
            <p>
                Tiket Anda sedang diproses oleh <strong>" . ($isEscalated ? "{$picEskalasi}" : "{$picTiket}") . "</strong>:
            </p>
        ";
        ...
        return $this->subject("Re: [Ticket #{$this->ticket->id_ticket}] {$this->ticket->ticket_title}")
                    ...
                    ->withSymfonyMessage(function ($message) {
                        $this->applyThreadHeaders($message, $this->ticket->id_ticket, false, 'onprogress');
                    });
    }
}